...
- **Week tabs — and every other button in the app — not responding to clicks.** First pass converted all 14 `.onclick =` assignments to `.addEventListener('click', ...)`, which turned out to be an incomplete fix: verified live that even freshly-added `addEventListener` listeners on the app's own buttons (created inside `render()`) sometimes don't receive real clicks in this browser environment, while a listener on `document` itself reliably does. Real fix: every button now goes through a `wireClick(el, fn)` helper that stores the handler as a plain property instead of registering a per-element DOM listener, backed by a single delegated `click` listener on `document` (installed once, never torn down) that looks up `[data-clickable]` and calls the stored handler. Verified against the actual bug with real clicks before redeploying, not just a syntax check.
